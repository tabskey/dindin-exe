...
- **Framework de testes no frontend (vitest)** — adiado na época: validação por `npm run build` +
  `npm run lint` + checklist de fluxo manual; **substituído pelo ADR 0005** (Vitest + Testing Library,
  Playwright e SonarQube local), decidido quando a lógica de UI merecedora de teste chegou (Fases 3+).
